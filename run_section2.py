"""Run Section 2 only: Low NA Circular Aperture (ε=0)
...
- 1D profiles (total, Ex, Ey, Ez) - symmetric r
- 2D x-y (contour + heatmap) - physical units, z=0 focal plane
- 2D u-v (contour + heatmap) - optical units
- 2D r-z (contour + heatmap) - axial cross-section, symmetric r
...
print("="*70)
print(f"λ = {wavelength} μm, NA = 0.1, ε = 0 (circular)")
print()
...
print("="*70)
print(f"\nOutput directory: {sec2_dir}")

# Count files
total_files = 0
for subdir in os.listdir(sec2_dir):
    subdir_path = f'{sec2_dir}/{subdir}'
