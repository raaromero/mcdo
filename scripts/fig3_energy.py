...
ax.plot(tau_ref, 0.11 * tau_ref + 0.54, 'k-', lw=1.2, label='0.11τ+0.54')

ax.set_xlabel('Pulse width (fsec)', fontsize=11)
ax.set_ylabel('Energy fraction', fontsize=11)
ax.set_title('Fig. 3 — Energy within central-spot FWHM vs τ', fontsize=11)
ax.set_xlim(0.5, 11.5)
all_vals = [v for v in results.values() if np.isfinite(v)]
ax.set_ylim(min(all_vals) - 0.02, max(all_vals) + 0.02)
ax.xaxis.set_major_locator(plt.MultipleLocator(1))
ax.legend(fontsize=7, ncol=3, loc='lower right', title='X_NA')
plt.tight_layout()
