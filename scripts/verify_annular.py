...
fig.suptitle('Phase 3 — annular apertures (field-level Babinet), linear-x y-cut',
             fontsize=12)

ax = axes[0, 0]
for e, c in zip(EPS, COLS):
    ax.plot(v_arr, tr_hi[e], color=c, lw=1.2, label=f'ε={e:g}')
ax.plot(v_arr, J0sq, 'k--', lw=1.4, label='J₀²(v) (Durnin)')
ax.set_yscale('log'); ax.set_ylim(1e-5, 1.5)
ax.set_xlabel('v'); ax.set_ylabel('normalized intensity')
ax.set_title('(a) transverse vs ε (X_NA=1.17): lobe narrows, sidelobes rise,\n'
             'ε→1 approaches the Bessel beam', fontsize=10)
ax.legend(fontsize=7.5, ncol=2)

ax = axes[0, 1]
for e, c in zip(EPS, COLS):
    ax.plot(u_arr, axial(cfg_at(XNA_HI), e), color=c, lw=1.0, label=f'ε={e:g}')
ax.set_xlim(0, 300); ax.set_yscale('log'); ax.set_ylim(1e-4, 1.5)
ax.set_xlabel('u (outer-aperture units)'); ax.set_ylabel('normalized intensity')
ax.set_title('(b) axial vs ε (X_NA=1.17): depth of focus extends', fontsize=10)
ax.legend(fontsize=7.5, ncol=2)

ax = axes[1, 0]
e_th = np.linspace(0, 0.992, 200)
ax.plot(e_th, 1 / (1 - e_th ** 2), 'k--', lw=1.4, label='theory 1/(1−ε²)')
for xna, st, lab in [(XNA_LO, 'C0o-', f'X_NA={XNA_LO} (sinα=0.1)'),
                     (XNA_HI, 'C3s-', f'X_NA={XNA_HI} (sinα=0.9)')]:
    ratios = np.array(dof[xna]) / dof[xna][0]
    ax.plot(EPS, ratios, st, ms=5, lw=1.2, label=lab)
ax.set_yscale('log')
ax.set_xlabel('ε (obstruction ratio)'); ax.set_ylabel('DOF(ε) / DOF(0)')
ax.set_title('(c) DOF extension: paraxial theory holds at low NA,\n'
             'falls short at high NA', fontsize=10)
ax.legend(fontsize=8)

ax = axes[1, 1]
ax.plot(v_arr, tr_hi[0.7], 'C0-', lw=1.3, label='uniform, ε=0.7')
ax.plot(v_arr, I_g, 'C3--', lw=1.3, label='Gaussian α_t=2, ε=0.7')
ax.plot(v_arr, tr_hi[0.0], 'k:', lw=1.2, label='uniform, full disk')
ax.set_yscale('log'); ax.set_ylim(1e-5, 1.5)
ax.set_xlabel('v'); ax.set_ylabel('normalized intensity')
ax.set_title('(d) illumination matters little once the annulus is thin:\n'
             'the ring passes a nearly uniform amplitude', fontsize=10)
ax.legend(fontsize=8)
