...
a common failure mode). Three checks:
  ...
  B. Isotropy/rotation correctness: scatter a +z beam ONCE with g=0 → the exit
     directions must be uniform on the sphere (⟨μz⟩≈0, ⟨μz²⟩≈1/3). A scalar-addition
...
# ── B. isotropy after one scatter (rotating the vector against adding scalars) ────
mu0 = np.tile(np.array([[0.0], [0.0], [1.0]]), (1, N))   # +z beam
cos_t, phi = hg_sample(0.0, N, rng)                       # isotropic
mu_correct = rotate(mu0, cos_t, phi)

# the incorrect form: theta0 = arctan2(z, sqrt(x^2+y^2)) (elevation), add deflection
...
print(f"   scalar add     : ⟨μz⟩={mu_bug[2].mean():+.4f}  ⟨μz²⟩={ (mu_bug[2]**2).mean():.4f}  (biased)")
...
ax.hist(mu_bug[2], bins=40, density=True, alpha=0.5, label='scalar addition')
