...
fig.suptitle('Two-photon excitation microscope, numerical aperture 0.8',
             fontsize=11)
...
ax.plot(tau_arr, F_tr, 'o', ms=7, mfc='white', mec='k', label='F (transverse)')
ax.plot(tau_arr, S_tr, 'o', ms=7, mfc='k',     mec='k', label='S (transverse)')
ax.axhline(1.0, color='gray', lw=1.2, ls=':')

tau_ref = np.logspace(0, 2, 100)
ax.plot(tau_ref, tau_ref ** 0.01,        'k-',  lw=1.6, label='1: F=τ⁰·⁰¹')
ax.plot(tau_ref, 0.97 * tau_ref ** 0.04, 'k--', lw=1.6, label='2: S=0.97τ⁰·⁰⁴')
...
ax.set_yticks(np.arange(0.965, 1.0051, 0.005))
