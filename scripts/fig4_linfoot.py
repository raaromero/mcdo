...
so the window scales with NA — this reproduces the paper's NA-insensitivity:
  transverse: v ∈ ±6.70   (= ±1 μm at X_NA=0.8, the paper's Fig-2 window)
  axial:      u ∈ ±24.74  (= ±6 μm at X_NA=0.8)

Validation at X_NA=0.8, τ=1 fs (full-spectrum ω-grid):
  ours: S_tr=1.053  S_ax=1.058  F_tr=0.963  F_ax=0.961
...
fig.suptitle('Fig. 4 — Single-photon microscope: Fidelity F and Structural Content S',
             fontsize=11)

ax = axes[0]
ax.plot(XNA_vals, F_ax_a, 's', ms=7, mfc='white', mec='k', label='F (axial)')
ax.plot(XNA_vals, S_ax_a, 's', ms=7, mfc='k',     mec='k', label='S (axial)')
ax.plot(XNA_vals, F_tr_a, 'o', ms=7, mfc='white', mec='k', label='F (transv)')
ax.plot(XNA_vals, S_tr_a, 'o', ms=7, mfc='k',     mec='k', label='S (transv)')
ax.axhline(1.0, color='gray', lw=0.8, ls=':')
ax.set_xlabel('NA', fontsize=10)
ax.set_ylabel('Criterion value', fontsize=10)
ax.set_title('(a)  τ = 1 fs', fontsize=10)
...
ax.plot(tau_arr, F_tr_b, 'o', ms=7, mfc='white', mec='k', label='F (transv)')
ax.plot(tau_arr, S_tr_b, 'o', ms=7, mfc='k',     mec='k', label='S (transv)')
ax.axhline(1.0, color='gray', lw=0.8, ls=':')

# Reference curves from the paper's caption (local fits near τ≈1 fs)
tau_ref = np.logspace(0, 2, 100)
ax.plot(tau_ref, 0.96 * tau_ref ** 0.1,  'k-',  lw=1, label='1: F=0.96τ⁰·¹')
ax.plot(tau_ref, 1.06 * tau_ref ** -0.1, 'k--', lw=1, label='2: S=1.06τ⁻⁰·¹')

ax.set_xscale('log')
ax.set_xlabel('Pulse width (fsec)', fontsize=10)
ax.set_ylabel('Criterion value', fontsize=10)
ax.set_title('(b)  X_NA = 0.8', fontsize=10)
ax.set_xlim(1, 100)
ax.set_ylim(0.95, 1.075)
ax.legend(fontsize=8, loc='center right')
