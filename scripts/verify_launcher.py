...
CONFIGS = [('uniform', None), ('Gaussian α_t=2', gaussian(2.0, cfg.sin2_alpha))]
COLOR = {'uniform': '#1f77b4', 'Gaussian α_t=2': '#d62728'}
MARKER = {'uniform': 'o', 'Gaussian α_t=2': 's'}
...
fig.suptitle('Phase 6a — photon initialization is sampled correctly '
             '(clear limit, no scattering)', fontsize=15, y=0.98)
...
ax_th.set_xlabel('launch angle θ (deg)'); ax_th.set_ylabel('density p(θ)')
ax_th.set_title(r'(1) DIRECTION: sampled $\circ$ vs expected —''\n'
                r'$p(\theta)\propto|A|\sqrt{\cos\theta}\,\sin\theta$')
ax_th.legend(loc='upper left')

ax_A.set_ylim(-0.05, 1.15)
ax_A.set_xlabel('launch angle θ (deg)'); ax_A.set_ylabel('recovered A(θ)')
ax_A.set_title('(2) INPUT amplitude: geometry divided out\n'
               r'→ flat (uniform) / Gaussian $e^{-\alpha_t\sin^2\theta/\sin^2\alpha}$')
ax_A.legend(loc='lower left')

ax_phi.axhline(1.0, color='k', ls='--', lw=1.4, zorder=0, label='expected (uniform)')
ax_phi.set_ylim(0.7, 1.3)
ax_phi.set_xlabel('azimuth φ (deg)'); ax_phi.set_ylabel('density / uniform')
ax_phi.set_title('(3) AZIMUTH: φ uniform on [0, 360°)\n(both inputs coincide)')
ax_phi.legend(loc='upper right', ncol=1)

ax_rad.set_ylim(-0.05, 1.15)
ax_rad.set_xlabel('pupil radius  ρ / ρ_max  (= sinθ/sinα)')
ax_rad.set_ylabel(r'recovered input intensity $|A|^2$')
ax_rad.set_title('(4) INPUT (2-D): radial intensity profile\n'
                 'flat disk (uniform) vs Gaussian')
...
for col, (label, apod) in enumerate(CONFIGS):
    ...
    ax.set_title(f'({5 + col}) 2-D pupil map |A|² — {label}')
...
fig2.suptitle('Phase 6a — intensity after MC launching matches the expected '
...
axt.set_title(r'transverse cut (z=0): MC $\circ$  vs expected —'); axt.legend()
axa.set_ylim(1e-3, 2); axa.set_xlabel('z (µm)'); axa.set_ylabel('intensity (norm.)')
axa.set_title(r'axial cut (x=0): MC $\circ$  vs expected —'); axa.legend()
...
for col, (label, apod) in enumerate(CONFIGS):
    ...
    ax.set_title(f'2-D gate — {label}\nMC heat map + expected iso-contours')
