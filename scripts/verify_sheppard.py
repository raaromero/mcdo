...
r_um = v_arr / (cfg.k_c * cfg.sin_alpha)      # physical radius (μm)
z_um = u_arr / (cfg.k_c * cfg.sin2_alpha)     # physical defocus (μm)
...
fig.suptitle('Gaussian-ring pupil vs Sheppard & Wilson 1978 '
             '(low NA; a fitted from focal, predicts axial)', fontsize=11)

ax = axes[0]
ax.plot(r_um, If, 'C0-', lw=2.0, label='RW Gaussian ring (low NA)')
ax.plot(r_um, focal_pred, 'k--', lw=2.0,
        label=f'Sheppard Eq.27: J₀²(v)exp(−a²v²), a={a_fit:.3f}')
ax.plot(r_um, J0sq, '0.7', lw=2.0, ls=':', label='J₀²(v) (a→0 δ-ring)')
ax.set_yscale('log'); ax.set_ylim(1e-4, 1.5)
ax.set_xlabel('Radial position r (μm)'); ax.set_ylabel('Normalized intensity')
ax.set_title('(a) Focal plane', fontsize=10); ax.legend(fontsize=8); trim_xlim(ax, floor=1e-4)

ax = axes[1]
ax.plot(z_um, Ia, 'C3-', lw=2.0, label='RW Gaussian ring (low NA)')
ax.plot(z_um, axial_pred, 'k--', lw=2.0,
        label=f'Sheppard Eq.33 (same a={a_fit:.3f})')
ax.set_xlabel('Axial position z (μm)'); ax.set_ylabel('Normalized on-axis intensity')
ax.set_title('(b) Axial profile', fontsize=10); ax.legend(fontsize=8); trim_xlim(ax, floor=1e-3)

plt.tight_layout()
out = os.path.join(OUT, 'verify_sheppard.png')
save_panels(fig, fig.axes, os.path.join(os.path.dirname(__file__), '..', 'output', '03_annular', 'panels'), 'verify_sheppard')
