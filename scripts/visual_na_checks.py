...
fig.suptitle('Check 1 — NA = 0.1: every theory must coincide with the Airy analytic',
             fontsize=11)
...
plt.tight_layout()
fig.savefig(os.path.join(OUT, 'check1_lowNA_convergence.png'), dpi=300,
...
plt.tight_layout()
fig.savefig(os.path.join(OUT, 'check2_highNA_vector.png'), dpi=300,
            bbox_inches='tight'); plt.close(fig)
...
ax.set_xlabel('NA'); ax.set_ylabel('FWHM (v units)')
...
ax.set_xlabel('NA'); ax.set_ylabel('|FWHM deviation| from scalar (%)')
ax.set_yscale('log'); ax.legend(fontsize=8)
ax.set_title('Vector effect magnitude (thresholds marked)', fontsize=10)
plt.tight_layout()
fig.savefig(os.path.join(OUT, 'check3_fwhm_thresholds.png'), dpi=300,
            bbox_inches='tight'); plt.close(fig)
...
ax.legend(fontsize=8)
plt.tight_layout()
fig.savefig(os.path.join(OUT, 'check4_axial.png'), dpi=300,
            bbox_inches='tight'); plt.close(fig)
...
fig, axes = plt.subplots(1, 2, figsize=(11, 4.4))
fig.suptitle('Check 5 — Gaussian apodization (NA=0.8): pupil profile → focal spot',
             fontsize=11)
ax = axes[0]
...
ax.legend(fontsize=8)
plt.tight_layout()
fig.savefig(os.path.join(OUT, 'check5_apodization.png'), dpi=300,
            bbox_inches='tight'); plt.close(fig)
...
ax.set_title('(a) Focal spot, NA=1.2 — elliptical', fontsize=10)
...
ax.set_title('(b) NA=1.2: E_z fills the zeros along x', fontsize=10)
ax.legend(fontsize=8)

ax = fig.add_subplot(1, 3, 3)
ax.plot(XNA_vals, ratio, 'C1o-', ms=4, lw=1.1, label='FWHM_x / FWHM_y')
ax.set_xlabel('NA'); ax.set_ylabel('FWHM anisotropy ratio', color='C1')
...
plt.tight_layout()
fig.savefig(os.path.join(OUT, 'check6_anisotropy.png'), dpi=300,
            bbox_inches='tight'); plt.close(fig)
