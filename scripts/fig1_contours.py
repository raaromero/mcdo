...
# Grid: r ∈ [-2.5, 2.5] μm, z ∈ [-6.5, 6.5] μm — the paper's Fig-1 aspect
# ratio (its axes read ±25 and ±65 μm, exactly ten times these; see below).
r_arr = np.linspace(-2.5, 2.5, 251)    # Δr = 0.02 μm
z_arr = np.linspace(-6.5, 6.5, 261)      # Δz = 0.05 μm
...
fig, axes = plt.subplots(2, 1, figsize=(8.6, 18.4))
fig.subplots_adjust(hspace=0.25)

RR, ZZ = np.meshgrid(r_arr, z_arr)

for ax, data, label in zip(axes, [I_cw_norm, I_pulsed_norm], ['(a) Continuous wave', '(b) 1 fs pulse']):
    cs = ax.contour(RR, ZZ, data, levels=levels, colors='k', linewidths=0.7)
    ax.contour(RR, ZZ, data, levels=[50.0], colors=['#ff7f0e'], linewidths=2.4)  # half maximum
    ax.clabel(cs, fmt='%.4g', fontsize=6, inline=True)
    ax.set_xlabel('Radial position r (μm)')
    ax.set_ylabel('Axial position z (μm)')
    ax.set_title(label)
    ax.set_xlim(r_arr[0], r_arr[-1])   # follow the grid, never a stale literal
...
save_panels(fig, axes, os.path.join(os.path.dirname(out), 'panels'), 'fig1_contours')
