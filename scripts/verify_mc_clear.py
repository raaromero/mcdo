...
fig.suptitle('Coherent MC in the clear limit reproduces the '
...
for ax, (data, ttl) in zip(axes[0], [
    ...
    ax.set_xlabel('x (μm)'); ax.set_ylabel('Axial position z (μm)'); ax.set_title(ttl, fontsize=10)

ax = axes[1, 0]
iz0 = np.argmin(np.abs(z_arr))
ax.plot(x_arr, ref[iz0], 'k-', lw=2.0, label='Debye')
ax.plot(x_arr, mc_by_N[1_000][iz0], 'C0:', lw=2.0, label='MC 10³')
ax.plot(x_arr, mc_by_N[100_000][iz0], 'C3--', lw=2.0, label='MC 10⁵')
...
ax.plot(z_arr, ref[:, ix0], 'k-', lw=2.0, label='Debye')
ax.plot(z_arr, mc_by_N[100_000][:, ix0], 'C3--', lw=2.0, label='MC 10⁵')
ax.set_xlabel('z (μm)  (x=0)'); ax.set_ylabel('I'); ax.legend(fontsize=8)
ax.set_title('Axial cut', fontsize=10)

ax = axes[1, 2]
ax.plot(v, airy_intensity(v), 'k-', lw=2.0, label='Airy')
ax.plot(v, I_lo, 'C3--', lw=2.0, label='MC 10⁵')
ax.set_yscale('log'); ax.set_ylim(1e-4, 1.5)
ax.set_xlabel('Optical radial coordinate v'); ax.set_ylabel('I'); ax.legend(fontsize=8)
ax.set_title('Low-NA (NA=0.3): MC = Airy', fontsize=10)
