"""
Hybrid Monte Carlo diffraction simulator.

Combines:
- Analytical jinc² sampling (no FFT discretization)
- Fraunhofer propagation formula (correct wavelength dependence)
"""
...
class AngularSpectrumHybrid:
    """
    Hybrid angular spectrum simulator.

    Uses analytical jinc²(k_ρ × R) sampling for k-space, then
    Fraunhofer diffraction formula for propagation to focal plane.

    This eliminates FFT discretization error while maintaining
    correct wavelength-dependent diffraction physics.
    ...
    def __init__(
        ...
        """Initialize the hybrid angular spectrum simulator."""
    ...
    def jinc_squared(self, k_rho: np.ndarray) -> np.ndarray:
        ...
        This is the angular spectrum intensity for a circular aperture.
    ...
    def sample_angular_spectrum(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Sample (kx, ky) from analytical jinc²(k_ρ × R) distribution.

        Uses rejection sampling for k_ρ, uniform sampling for θ.
        ...
        """
        # Sample k_ρ from k_ρ × jinc²(k_ρ × R)
        k_rho = self.sample_k_rho_rejection(self.n_photons)
    ...
    def propagate(self) -> dict:
        ...
        Uses:
        1. Analytical jinc² sampling (no FFT discretization)
        2. Fraunhofer diffraction formula (wavelength-dependent)
        ...
        # x = (λf / 2π) × kx
        # y = (λf / 2π) × ky
        # Wavelength is a crucial factor!
