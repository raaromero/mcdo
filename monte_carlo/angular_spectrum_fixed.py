"""
Monte Carlo simulation using angular spectrum sampling - FIXED VERSION.

Fixed: Use direct Fraunhofer formula for k-space to focal plane mapping
instead of ray tracing approximation.
"""
...
class AngularSpectrumSimulatorFixed:
    """
    Monte Carlo simulator using angular spectrum approach - FIXED.
    ...
    FIX: Uses direct Fraunhofer diffraction formula:
         x = (λf)/(2π) × kx
    ...
    def propagate(self) -> dict:
        ...
        Physics (Angular Spectrum Method - FIXED):
        ...
        4. Use Fraunhofer diffraction formula (NOT ray tracing):
           x = (λf)/(2π) × kx
           y = (λf)/(2π) × ky

        This is the exact Fraunhofer formula relating k-space to focal plane position.
